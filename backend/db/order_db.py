...
class OrderDB:
    ...
    def create_order(user_id: int, total_amount: float, shipping_address: str, status: str = "pending") -> dict:
        ...
        try:
            query = "INSERT INTO orders (user_id, total_amount, status, shipping_address, created_at) VALUES (%s, %s, %s, %s, NOW()) RETURNING id, user_id, total_amount, status"
            cursor.execute(query, (user_id, total_amount, status, shipping_address))
            conn.commit()
            result = cursor.fetchone()
            return {"id": result[0], "user_id": result[1], "total_amount": result[2], "status": result[3]}
        finally:
            cursor.close()
    ...
    def get_order_by_id(order_id: int) -> Optional[dict]:
        ...
        try:
            query = "SELECT id, user_id, total_amount, status, shipping_address, created_at FROM orders WHERE id = %s"
    ...
    def get_user_orders(user_id: int, limit: int = 10, offset: int = 0) -> Tuple[list, int]:
        ...
        try:
            query = "SELECT id, user_id, total_amount, status, shipping_address, created_at FROM orders WHERE user_id = %s ORDER BY created_at DESC LIMIT %s OFFSET %s"
    ...
    def get_all_orders(limit: int = 10, offset: int = 0) -> Tuple[list, int]:
        ...
        try:
            query = "SELECT id, user_id, total_amount, status, shipping_address, created_at FROM orders ORDER BY created_at DESC LIMIT %s OFFSET %s"
